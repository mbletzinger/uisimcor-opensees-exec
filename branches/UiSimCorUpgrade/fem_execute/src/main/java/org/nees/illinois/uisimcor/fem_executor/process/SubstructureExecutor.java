...
public class SubstructureExecutor {
	...
	public final void startSimulation() {
		...
		try {
			...
			pm.getStdinQ().add(new QMessageT(QMessageType.Setup, init));
		} catch (IOException e) {
	...
	public final void startStep(final int step, final double[] displacements) {
		...
		pm.getStdinQ().add(new QMessageT(QMessageType.Command, stepCmnd));
		debugCnt = 0;
	}
	...
	public final boolean stepIsDone() throws OutputFileException {
		BlockingQueue<QMessageT> responses = pm.getResponseQ();
		QMessageT rspStr = responses.poll();
		if (rspStr == null) {
			if (debugCnt == maxCnt) {
				debugCnt = 0;
